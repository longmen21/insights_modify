...
class BaseGeolocation(object):
    ...
    def reducer(self, key, values):
        ...
        try:
            ...
            # edited
            tmp = self.geoip.record_by_addr(last_ip)
            city = tmp['city']
        ...
        if city is None or len(city.strip()) <= 0:
            log.error("No city found for user '%s', last_ip '%s', country '%s' on '%s'.",
                      username, last_ip, country, last_timestamp)
            # TODO: try earlier IP addresses, if we find this happens much.
            city = UNKNOWN_CITY
        ...
        # edited
        yield (country.encode('utf8'), code.encode('utf8'), city.encode('utf8')), username.encode('utf8')
...
class UsersPerCountry(MapReduceJobTask, BaseUserLocationTask):
    ...
    def mapper(self, line):
        ...
        # edited
        country, code, _username, city = line.split('\t')
        if len(country) > 0:
            # yield (country, code), 1
            # edited
            yield (country, code, city), 1
...
class UsersPerCountryReport(luigi.Task):
    ...
    def create_header(cls, date):
        ...
        # edited
        fields = ['percent', 'count', 'country', 'code', 'city', 'date={date}'.format(date=date)]
        return ','.join(fields)

    @classmethod
    def create_csv_entry(cls, percent, count, country, code, city):
        ...
        # )
        # edited
        return '{percent:.4f},{count},"{country}",{code},"{city}"'.format(
            percent=percent, count=count, country=country, code=code, city=city
        )
    ...
    def run(self):
        ...
        with self.input().open('r') as input_file:
            for line in input_file.readlines():
                ...
                # edited
                country, code, count, city, date = line.split('\t')
                # counts.append((count, country, code))
                counts.append((count, country, code, city))
                date = date.strip()
                total += int(count)
        ...
        with self.output().open('w') as output_file:
            ...
            # edited
            for count, country, code, city in sorted(counts, reverse=True, key=lambda k: int(k[0])):
                percent = float(count) / float(total)
                # output_file.write(self.create_csv_entry(percent, count, country, code))
                # edited
                output_file.write(self.create_csv_entry(percent, count, country, code, city))
                output_file.write('\n')
